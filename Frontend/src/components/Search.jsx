...
const Search = () => {
  ...
  const handleChange = async (e) => {
    ...
    let { data } = await axios.get(`/user/search?username=${query}`);
    setUsers(data.data);
    setLoading(false);
  };
  return (
    <div className=" min-h-screen bg-black ">
      <div className="flex items-center justify-center gap-1 py-2 border-b-[1px] border-neutral-600 
      sm:justify-between
      ">
        <div className=" gap-2 border-2 border-neutral-600 rounded-md pl-2 py-[2px] flex w-[76%] items-center justify-between
        sm:w-[90%] sm:my-5
        ">
          ...
          <div className=" relative w-5">
            {!loading  ? query && <RiCloseCircleFill
              onClick={() => {
                setQuery("");
                setUsers([]);
              }}
